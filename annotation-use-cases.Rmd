...
We next annotate these chromatographic peaks based only using their m/z
values. As a reference we load a `CompDb` database containing all compounds from
the Human Metabolom Database [@wishartHMDBHumanMetabolome2018] version 5.0 which
is also provided within this repository. Such annotation resources can be
created with the `r Biocpkg("CompoundDb")` package (see the [package
documentation](https://euracbiomedicalresearch.github.io/CompoundDb) for more
information).
...
compound. Thus, in order to annotate our chromatographic peaks' m/z values, m/z
values for expected ions of the compounds need to be calculated first. With
`Mass2MzParam` we can configure the parameters for the annotation. Parameter
`adducts` allows to specify the adducts (ions) we expect to see mostly in the
data set. For our example we assume to see mostly `[M+H]+` and `[M+Na]+`
positive ions. With parameters `tolerance` and `ppm` we can finaly define also
the absolute and m/z relative acceptable difference for m/z values to be
considered matching.
...
spectra similarities (calculated with the normalized dotproduct) higher than 0.7
